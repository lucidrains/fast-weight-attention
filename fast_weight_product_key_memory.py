...
class ChunkManager(Module):
    ...
    @property
    def chunk_size(self):
        return self.net.chunk_size

    @property
    def init_memories(self):
        return self.net.init_memories
    ...
    def forward(
        ...
        for chunk_index, segment in enumerate(segments):
            ...
            if past_mem.num_cached == chunk_size:
                _, store_inter = self.net.retrieve(
                    cat((past_mem.cached_tokens, get_first_token(segment)), dim = 1),
                    past_memories = past_mem,
                    idw_eps = idw_eps
                )

                mv, mk = self.net.store(store_inter, past_memories = past_mem, detach_next_memories = should_detach, idw_eps = idw_eps)
                past_mem = past_mem._replace(memory_values = mv, keys = mk, cached_tokens = None, num_cached = 0)

            # retrieve outputs

            out, inter = self.net.retrieve(
        ...
        if exists(detach_next_memories_every) and should_detach:
...
def fwPKM(*args, **kwargs):
    return ChunkManager(fwPKMCore(*args, **kwargs))
